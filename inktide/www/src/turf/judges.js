...
export class JudgeBooth {
  ...
  _buildBooth(height) {
    ...
    G.add(riser);
    // striped canopy
    const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 3.2, 8), dark);
    pole.position.set(0, 1.6, -0.9);
    G.add(pole);
    const canopy = new THREE.Mesh(new THREE.ConeGeometry(2.4, 0.8, 12, 1, true), red);
    canopy.position.set(0, 3.35, -0.5);
    canopy.material.side = THREE.DoubleSide;
    G.add(canopy);
    const tip = new THREE.Mesh(new THREE.SphereGeometry(0.12, 10, 8), white);
    tip.position.set(0, 3.8, -0.5);
    G.add(tip);
  ...
  async _spawnOtters() {
    ...
    for (const [who, x] of [['otto', -0.85], ['tilly', 0.85]]) {
      ...
      flag.visible = false;
      if (arm) { flag.position.set(0, -0.3, 0.04); flag.scale.setScalar(1.25); arm.add(flag); } else { flag.position.set(x - 0.3, 0.9, 0.5); this.group.add(flag); }
  ...
  reveal(color) {
    ...
    for (const o of this.otters) {
      o.m.emote?.('cheer', 4);
      if (o.m.flag?.material) { o.m.flag.material.color.set(color); }
    }
  }
  ...
  update(dt) {
    this.t += dt;
    for (const o of this.otters) {
      o.m.update?.(dt, { talking: this.state === 'drum' });
      if (!o.arm) continue;
      if (this.state === 'drum') {
        // tapping the desk
